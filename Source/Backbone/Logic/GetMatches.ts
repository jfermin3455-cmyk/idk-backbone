...
async function GenerateSingleElimination(
  ...
  for (let Round = 1; Round <= TotalRounds; Round++) {
    // ✅ FIX: عدد الماتشات = عدد الفرق / (MaxPlayersPerMatch / PartySize)
    const NumTeamsPerMatch = Math.floor(Tournament.MaxPlayersPerMatch / Tournament.PartySize);
    const MatchCount = Math.ceil(TeamsRemaining / NumTeamsPerMatch);
    ...
    for (let MatchNum = 1; MatchNum <= MatchCount; MatchNum++) {
      ...
      if (Round === 1) {
        ...
        // ✅ FIX: عدد الفرق = MaxPlayersPerMatch / PartySize
        const NumTeamsInMatch = Math.floor(Tournament.MaxPlayersPerMatch / Tournament.PartySize);
        
        for (let Slot = 0; Slot < NumTeamsInMatch; Slot++) {
          const TeamIndex = (MatchNum - 1) * NumTeamsInMatch + Slot;
        ...
        // ✅ FIX: المقارنة مع عدد الفرق الصحيح
        if (ValidTeams === NumTeamsInMatch) {
    ...
    TeamsRemaining =
      Tournament.MinPlayersPerMatch === Tournament.MaxPlayersPerMatch && Tournament.MaxPlayersPerMatch === 2
        ? MatchCount
        : MatchCount * Math.floor(Tournament.MinPlayersPerMatch / Tournament.PartySize);
...
async function GenerateRoundRobinGroup(
  ...
  for (let Round = 1; Round <= TotalRounds; Round++) {
    ...
    // ✅ FIX: عدد الماتشات = عدد الفرق / (MaxPlayersPerMatch / PartySize)
    const NumTeamsPerMatch = Math.floor(Tournament.MaxPlayersPerMatch / Tournament.PartySize);
    const MatchCount = Math.ceil(MaxPossiblePlayers / NumTeamsPerMatch);
    ...
    for (let MatchNum = 0; MatchNum < MatchCount; MatchNum++) {
      ...
      if (Round === 1) {
        const TeamsInMatch: TeamUser[][] = [];
        for (let Slot = 0; Slot < NumTeamsPerMatch; Slot++) {
          const TeamIndex = MatchNum * NumTeamsPerMatch + Slot;
      ...
      if (Round === 1) {
        if (ValidTeamCount === NumTeamsPerMatch) {
          Status = TournamentMatchStatus.GameReady;
